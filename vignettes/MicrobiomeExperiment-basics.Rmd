...
Concentration of microbiotic organisms in samples can be measured various 
methods, such as qPCR or cell counts. If you want to store such information per
sample `sampleConcentrations` is the right function to use. 

```{r}
sampleConcentrations(me) <- runif(ncol(me))
sampleConcentrations(me)
```

It will store the information as `colData` and can be used in different 
contexts. ToDo
